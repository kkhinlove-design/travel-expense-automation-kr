...
| 운영 | 이메일/비밀번호 로그인 · 개인별 기본 출발 기준지 · 직원 계정 생성·편집·Excel 등록 · 공용 운임 Excel/Google 시트 등록 · 승인/정산 출장대장 · 사용자별 기록 저장·삭제 |
...
새 Supabase 프로젝트를 만들고 [Supabase 설정 안내](./docs/SUPABASE_SETUP.md)에 따라 데이터베이스 마이그레이션, Edge Functions와 최초 관리자 계정을 준비합니다. 마이그레이션의 비공개 Storage 버킷은 기존 버전 호환용이며 현재 앱은 새 원본 파일을 저장하지 않습니다.
...
| `FARE_SOURCE_GOOGLE_SHEET_ID` | 아니요 | 서버 | 관리자 운임 가져오기에 사용할 링크 공개 Google 시트 ID. 비우면 프로젝트 기본 시트를 사용합니다. |
| `FARE_SOURCE_GOOGLE_SHEET_GID` | 아니요 | 서버 | Google 시트의 운임 탭 gid. 비우면 프로젝트 기본 탭을 사용합니다. |

기관명·앱 이름·출발 기준지와 Ollama 모델 등 선택 설정은 주석이 포함된 [`.env.example`](./.env.example)에서 확인할 수 있습니다. 여러 기준지는 `NEXT_PUBLIC_ORIGIN_BASES`, 복명서 결재자 직위는 `NEXT_PUBLIC_REPORT_APPROVER_TITLES`에 쉼표로 구분해 입력합니다. 직원은 첫 로그인 때 **환경 설정**에서 기본 출발 기준지와 `1차 결재자 → 최종 결재자`를 먼저 저장하며, 출장마다 실제 출발지가 다르면 정보 확인 화면에서 바꿀 수 있습니다. `ALLOW_LOCAL_DEV_USER=true`는 Supabase 없이 UI를 확인하는 로컬 개발에서만 사용하고 Vercel에는 설정하지 마세요.

관리자 운임 설정은 링크 공개 Google 시트 또는 Excel을 미리보기로 읽은 뒤 공용 기준표에 반영합니다. 자동 계산은 출발지→도착지 정방향 노선을 우선하고, 정방향이 없을 때만 도착지→출발지 노선의 가는 길·오는 길 운임을 바꾸어 적용합니다. 비공개 Google 시트 OAuth 연동은 제공하지 않습니다.
...
- PDF는 브라우저에서 분석합니다. HWPX는 표 오추출을 줄이기 위해 브라우저 분석과 함께 로그인한 앱의 Vercel API에서 Kordoc으로 일시 분석하며, 분석 요청 자체는 저장하지 않습니다.
- PDF/HWPX는 파싱할 때만 사용하고 원본 파일은 저장하지 않습니다. 추출된 출장일·출장자·출장지·문서번호와 정산 상태만 자신의 Supabase 프로젝트에 기록되며, 최종 저장 후 같은 기록이 `정산 완료`로 전환됩니다.
