  ...
  return (
    <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 24, background: "#f4f6fa" }}>
      <form onSubmit={submit} style={{ width: "min(100%, 420px)", padding: 32, borderRadius: 18, background: "#fff", boxShadow: "0 14px 50px #1d2b4418" }}>
        ...
        {!reset && <label htmlFor="signin-email" style={{ display: "block", marginTop: 16, color: "#344054", fontWeight: 700 }}>이메일</label>}
        {!reset && <input id="signin-email" required type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" autoComplete="email" style={{ width: "100%", boxSizing: "border-box", margin: "8px 0 12px", padding: 13, border: "1px solid #ccd4e0", borderRadius: 8 }} />}
        {reset ? (
          <><label htmlFor="signin-new-password" style={{ display: "block", marginTop: 16, color: "#344054", fontWeight: 700 }}>새 비밀번호</label><input id="signin-new-password" required minLength={8} type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="8자 이상" autoComplete="new-password" style={{ width: "100%", boxSizing: "border-box", margin: "8px 0 16px", padding: 13, border: "1px solid #ccd4e0", borderRadius: 8 }} /></>
        ) : (
          <><label htmlFor="signin-password" style={{ display: "block", color: "#344054", fontWeight: 700 }}>비밀번호</label><input id="signin-password" required minLength={8} type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" style={{ width: "100%", boxSizing: "border-box", margin: "8px 0 16px", padding: 13, border: "1px solid #ccd4e0", borderRadius: 8 }} /></>
        )}
