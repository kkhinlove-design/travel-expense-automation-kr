...
| 출장 기록 | 기관의 Supabase | 출장일·출장자·출장지·문서번호·정산 상태와 작성 정보 |
...
- 기존 버전에서 보관된 원본이 있다면 비공개 버킷과 사용자별 경로 정책을 유지합니다. 현재 버전은 새 원본을 저장하지 않습니다.
...
- PDF/HWPX 원본은 파싱할 때만 사용하며 새 원본 파일을 Supabase Storage에 저장하지 않습니다.
- 추출된 출장일·출장자·출장지·문서번호와 작성 정보는 누락 점검용 출장대장에 `정산 필요`로 기록되고, 최종 저장 후 `정산 완료`로 구분됩니다. 기록은 본인 계정에서만 조회·삭제할 수 있습니다.

## 보존과 삭제

앱의 삭제 기능은 저장된 출장 기록을 삭제합니다. 기존 버전에서 연결된 원본이 있는 경우에만 함께 정리합니다. 운영자는 다음도 별도로 결정해야 합니다.

- 법령과 기관 규정에 따른 보존기간
- Supabase 백업과 로그에 남는 기간
- 퇴직자 계정의 비활성화·세션 해제·자료 이관 절차
- 기존 버전에서 남은 Storage 객체의 보존·파기 기준
