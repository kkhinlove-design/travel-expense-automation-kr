...
// 조회일과 노선 조합마다 캐시 항목이 생기므로 상한과 만료 정리가 없으면
// 프로세스가 살아 있는 동안 Map이 계속 커진다.
...
function pruneMemoryCache(now) {
  ...
  // Map은 삽입 순서를 지키므로 가장 오래 전에 넣은 항목부터 버린다.
...
function rememberInMemory(key, value, ttlSeconds, now) {
...
async function cachedJson(key, ttlSeconds, loader) {
  ...
  if (edgeCache) {
    ...
    if (cached) {
      ...
      rememberInMemory(key, value, ttlSeconds, now);
      return value;
    }
  }
