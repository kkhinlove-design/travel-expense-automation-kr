...
import { normalizeTripWaypoints, tripRoutePoints, tripTransportFares } from "./travel-rules.js";
...
function insertWorksheetSetting(document, element) {
  ...
  const firstTail = [...document.documentElement.children].find((child) => tailNames.includes(child.localName));
  document.documentElement.insertBefore(element, firstTail ?? null);
}
...
export async function buildTravelWorkbook(trip, expense) {
  ...
  const outboundGrade = trip.fareSources?.outbound?.grade || (!hasDirectionalSources ? trip.fareSource?.grade : null) || "-";
  const returnGrade = trip.fareSources?.return?.grade || (!hasDirectionalSources ? trip.fareSource?.grade : null) || "-";
