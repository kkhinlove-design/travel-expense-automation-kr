...
import { reportApprovalLineForDocument } from "./travel-user-preferences.js";
...
async function appendWrappedCellStyle(zip, sourceStyleIndex) {
  const stylesPath = "xl/styles.xml";
  const stylesFile = zip.file(stylesPath);
  if (!stylesFile) throw new Error("Excel 셀 서식 정보를 찾지 못했습니다.");
  const document = xml(await stylesFile.async("string"));
  const cellXfs = document.getElementsByTagNameNS(XML_NS, "cellXfs")[0];
  const styles = cellXfs ? [...cellXfs.children].filter((child) => child.localName === "xf") : [];
  if (!cellXfs || !styles.length) throw new Error("Excel 셀 서식을 읽지 못했습니다.");

  const safeSourceIndex = Number.isInteger(sourceStyleIndex) && styles[sourceStyleIndex] ? sourceStyleIndex : 0;
  const wrappedStyle = styles[safeSourceIndex].cloneNode(true);
  let alignment = [...wrappedStyle.children].find((child) => child.localName === "alignment");
  if (!alignment) {
    alignment = node(document, "alignment");
    const next = [...wrappedStyle.children].find((child) => ["protection", "extLst"].includes(child.localName));
    wrappedStyle.insertBefore(alignment, next ?? null);
  }
  alignment.setAttribute("wrapText", "1");
  alignment.setAttribute("vertical", "top");
  wrappedStyle.setAttribute("applyAlignment", "1");
  cellXfs.appendChild(wrappedStyle);
  cellXfs.setAttribute("count", String(styles.length + 1));
  zip.file(stylesPath, new XMLSerializer().serializeToString(document));
  return styles.length;
}

async function setReportContentLayout(zip, sheetName, lines) {
  const path = await sheetPath(zip, sheetName);
  const document = xml(await zip.file(path).async("string"));
  const sheetData = document.getElementsByTagNameNS(XML_NS, "sheetData")[0];
  const firstCell = findOrCreateCell(document, findOrCreateRow(document, sheetData, 12), "B12");
  const sourceStyleIndex = Number(firstCell.getAttribute("s"));
  const wrappedStyleIndex = await appendWrappedCellStyle(zip, sourceStyleIndex);

  for (let rowNumber = 12; rowNumber <= 23; rowNumber += 1) {
    const row = findOrCreateRow(document, sheetData, rowNumber);
    const cell = findOrCreateCell(document, row, `B${rowNumber}`);
    cell.setAttribute("s", String(wrappedStyleIndex));
    row.setAttribute("ht", String(reportContentRowHeight(lines[rowNumber - 12] ?? "")));
    row.setAttribute("customHeight", "1");
  }
  zip.file(path, new XMLSerializer().serializeToString(document));
}
...
async function setSheetPrintLayout(zip, sheetName, { orientation, margins, centered = false, fitOnePage = false }) {
  ...
  if (fitOnePage) {
    let sheetPr = document.getElementsByTagNameNS(XML_NS, "sheetPr")[0];
    if (!sheetPr) {
      sheetPr = node(document, "sheetPr");
      insertWorksheetSetting(document, sheetPr);
    }
    let pageSetUpPr = [...sheetPr.children].find((child) => child.localName === "pageSetUpPr");
    if (!pageSetUpPr) {
      pageSetUpPr = node(document, "pageSetUpPr");
...
const REPORT_CONTENT_ROWS = 12;
const REPORT_ROW_BASE_HEIGHT = 24.75;
const REPORT_LINE_HEIGHT = 15;
const REPORT_LINE_WIDTH_UNITS = 72;
const EXCEL_MAX_ROW_HEIGHT = 409;

function reportLineWeight(line) {
  return Math.max(1, [...String(line)].reduce((total, character) => (
    total + (character.codePointAt(0) > 0x7f ? 2 : 1)
  ), 0));
}

/**
 * 기존 B12:B23 셀 범위는 그대로 두고, 입력 줄을 최대 12개의 연속 묶음으로
 * 균형 있게 나눈다. 묶음을 다시 줄바꿈으로 합치면 정규화된 원문과 같아야 하며
 * 빈 줄도 삭제하지 않는다.
 */
export function packReportLinesForSheet(value, maxRows = REPORT_CONTENT_ROWS) {
  const normalized = String(value ?? "").replace(/\r\n?/g, "\n");
  if (!normalized) return [];
  const rowLimit = Math.max(1, Math.floor(Number(maxRows) || REPORT_CONTENT_ROWS));
  const sourceLines = normalized.split("\n");
  if (sourceLines.length <= rowLimit) return sourceLines;

  const weights = sourceLines.map((line) => reportLineWeight(line) + 1);
  const groups = [];
  let cursor = 0;
  let remainingWeight = weights.reduce((total, weight) => total + weight, 0);
  for (let slot = 0; slot < rowLimit; slot += 1) {
    const remainingSlots = rowLimit - slot;
    const targetWeight = remainingWeight / remainingSlots;
    const latestCursor = sourceLines.length - (remainingSlots - 1);
    const group = [];
    let groupWeight = 0;
    while (cursor < latestCursor) {
      if (group.length && groupWeight >= targetWeight) break;
      group.push(sourceLines[cursor]);
...
export function reportContentRowHeight(value) {
  const lines = String(value ?? "").replace(/\r\n?/g, "\n").split("\n");
  const visualLines = lines.reduce((total, line) => (
    total + Math.max(1, Math.ceil(reportLineWeight(line) / REPORT_LINE_WIDTH_UNITS))
  ), 0);
  return Math.min(EXCEL_MAX_ROW_HEIGHT, Math.max(REPORT_ROW_BASE_HEIGHT, visualLines * REPORT_LINE_HEIGHT));
}

function reportLines(trip) {
  const entered = String(trip.reportContent ?? "").replace(/\r\n?/g, "\n");
  const source = entered.trim() ? entered : buildRuleBasedTravelReport(trip);
  return packReportLinesForSheet(source);
}
...
export async function buildTravelWorkbook(trip, expense) {
  ...
  const reportPatches = {
    G3: reportApprovalLine[0],
    H3: reportApprovalLine[1],
  ...
  await setReportContentLayout(zip, "출장복명서", lines);
