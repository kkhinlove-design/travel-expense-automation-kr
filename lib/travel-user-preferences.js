...
export async function loadTravelUserPreference(client, userId) {
  if (!client || !userId) return { defaultOrigin: "", reportApprovalLine: [...ORGANIZATION_CONFIG.defaultReportApprovalLine], error: null };
  const { data, error } = await client
    .from(TRAVEL_USER_PREFERENCES_TABLE)
    .select("default_origin,report_approver_first,report_approver_second")
    .eq("user_id", userId)
    .maybeSingle();

  if (error) return { defaultOrigin: "", reportApprovalLine: [...ORGANIZATION_CONFIG.defaultReportApprovalLine], error };
  return {
    defaultOrigin: allowedOriginPreference(data?.default_origin),
    reportApprovalLine: initialReportApprovalLine([
